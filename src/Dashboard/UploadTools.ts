...
export function addNewFiles( files: File[] ): File[] {
  const file_container: HTMLDivElement = document.querySelector('#uploading_list');
...
async function uploadDoc(id: string, name: string, file: File, type: string, currentFolder: IFolder): Promise<boolean> {
  const newName = fnConflictHandler(name, FileSystemTools.getAllNames(currentFolder));
  return createJson(file, type)
    .then(jsonBlob => {
    ...
    .then(succeeded => {
      if (succeeded) {
        ...
        const docEntry = FileSystemTools.addMetadata(fileEntry, { created_on: datetime });
